...
class yolo_fast_v2():
    def __init__(self, objThreshold=0.3, confThreshold=0.3, nmsThreshold=0.4):
        with open('coco.names_final', 'rt') as f:
        ...
        self.objdetec_oneframe = {} #{'person':(345, 452, 0.54)}
    
    def get_classID(self):
        return self.objdetec_oneframe
    ...
    def postprocess(self, frame, outs):
        ...
        # print("frameHeight = ", end='')
        # print(frameHeight)
        # print("frameWidth = ", end='')
        # print(frameWidth)

        ratioh, ratiow = frameHeight / self.inpHeight, frameWidth / self.inpWidth

        # print("self.inpHeight = ", end='')
        # print(self.inpHeight)
        # print("self.inpWidth = ", end='')
        # print(self.inpWidth)
        ...
        for detection in outs:
            ...
            if confidence > self.confThreshold and detection[4] > self.objThreshold:
                ...
                # self.objdetec_oneframe.update({classIds[]:[center_x, center_y]})

        # Perform non maximum suppression to eliminate redundant overlapping boxes with
        # lower confidences.
        indices = cv2.dnn.NMSBoxes(boxes, confidences, self.confThreshold, self.nmsThreshold)
        self.objdetec_oneframe.clear() #clear the objects in last frame
        for i in indices:
            i = i[0]
            box = boxes[i]
            left = box[0]
            ...
            width = box[2]
            height = box[3]
            self.objdetec_oneframe.update({self.classes[classIds[i]]:(int(left + width / 2), int(top + height / 2), round(((left + width / 2)/frameWidth), 2))}) #save the obj detected in one frame
    ...
    def detect(self, srcimg):
        blob = cv2.dnn.blobFromImage(srcimg, 1 / 255.0, (self.inpWidth, self.inpHeight)) #1 / 255.0
